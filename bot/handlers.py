...
from core.repository import (
    ...
    load_budget, load_month_expenses, load_category_buckets,
)
from core.budget import compute_balance
...
async def handle_expense(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if default_type:
        try:
            ...
            await update.message.reply_text("❌ Couldn't save the expense. Check logs.")
        return ConversationHandler.END
...
async def handle_type_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.edit_message_text("❌ Couldn't save the expense. Check logs.")
...
async def balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    budget = load_budget()
    if not budget:
        await update.message.reply_text("❌ Couldn't load budget. Check your budget settings.")
        return

    expenses = load_month_expenses()
    type_bucket = load_category_buckets()
    result = compute_balance(budget, expenses, type_bucket)
    ...
    for cat, data in result.items():
        ...
        total_spent += s
        bar = "🟢" if r >= 0 else "🔴"
        rem_label = "Over" if r < 0 else "Remaining"
        lines.append(
            f"{bar} *{cat}*\n"
            f"  Budget:    ${b:,.2f}\n"
            f"  Spent:     ${s:,.2f}\n"
            f"  {rem_label}: ${abs(r):,.2f}\n"
        )

    total_remaining = total_budget - total_spent
    total_bar = "🟢" if total_remaining >= 0 else "🔴"
    total_label = "Over" if total_remaining < 0 else "Remaining"
    lines.append(
        f"─────────────────\n"
        f"{total_bar} *Total*\n"
        f"  Budget:    ${total_budget:,.2f}\n"
        f"  Spent:     ${total_spent:,.2f}\n"
        f"  {total_label}: ${abs(total_remaining):,.2f}"
    )
